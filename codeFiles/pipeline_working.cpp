...
void execute(){
    ...
    // store instructions execution
        ...
        int newImmed= (~((~0)<<8))&P_DE_EX.immd;
        aluop_result = P_DE_EX.op1 + newImmed;
        ...
        aluop_result = P_DE_EX.op1 + P_DE_EX.immd;
    ...
    //lui and auipc
        ...
        aluop_result=P_DE_EX.immd<<12;
    }
    else if(P_DE_EX.alu_op=="auipc"){
        aluop_result=pc+(P_DE_EX.immd<<12);
    }
    else{
    ...
    P_EX_MA.alu_result = aluop_result;
    P_EX_MA.pc = pc;                                //--
...
void memory(){
    //laod 
    cout<<"\nACCESSING MEMORY ... "<<P_EX_MA.instruction<<'\n';
